...
class PaymentGatewayController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $rules = array_merge($paypal_rules, $stripe_rules, $bt_rules, $pt_rules, $fw_rules, $rp_rules);
        ...
        if ($request->stripe_enabled && $request->bt_enabled && $request->paytm_enabled && $request->razorpay_enabled) {
            flashMessage('danger', 'Please Choose either Braintree or Stripe or Razorpay or Paytm for Card Payments');
            return back();
        }

        if ($request->stripe_enabled == '0' && $request->bt_enabled == '0' && $request->paypal_enabled == '0' && $request->paytm_enabled == '0' && $request->razorpay_enabled == '0') {
        ...
        if (in_array('stripe', $default_payments)) {
            if ($request->stripe_enabled == "0" && $request->bt_enabled == "0" && $request->Paytm_enabled == "0" && $request->razorpay_enabled == '0') {
                flashMessage('danger', 'Please enable Stripe or Braintree or Razorpay or Paytm. Because card is default payment method');
                return back();
            }
        }
