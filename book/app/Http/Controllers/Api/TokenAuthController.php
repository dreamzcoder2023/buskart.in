...
//         $rules = array(
//         'phone' => 'required|exists:companies,mobile_number',
//         'password' => 'required',
//     );
...
//     ];
//   try {
//         // Attempt authentication using JWTAuth
...
public function company_login(Request $request){
     $user_id = $request->phone;
        $auth_column = 'mobile_number';
     $rules = array(
        'phone' => 'required|exists:users,mobile_number',
        'password' => 'required',
    );
       $credentials = [
        'mobile_number' => $request->phone,
    ...
    \Log::info('Attempting to log in', $credentials);
   
    $attempt = Auth::attempt([$auth_column=>$user_id, 'password'=>$request->password, 'user_type'=>'Company']);
