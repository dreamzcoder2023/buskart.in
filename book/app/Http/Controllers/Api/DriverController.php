	...
	public function updateLocation(Request $request)
	{
		...
		if($driver_location && $driver_location->status == "Pool Trip") {
			...
			if($request->status == "Online"){
			   $loc = DriverLocation::where('user_id', $user_details->id)->first();

// Get the current date and time in the 'Asia/Kolkata' timezone
$currentDateTime = Carbon::now('Asia/Kolkata');

// Compare the stored online date with the current date and time
if ($loc->online_date && Carbon::parse($loc->online_date)->isSameDay($currentDateTime)) {
     $data['online_date'] = Carbon::now('Asia/Kolkata')->format('Y-m-d H:i:s');
}else{
    $data['total_hour'] = 0;
    $data['online_date'] = Carbon::now('Asia/Kolkata')->format('Y-m-d H:i:s');
	}
    ...
    // Fetch the user's online time from the database
    $driverLocation = DriverLocation::where('user_id', $user_details->id)->first();
    $onlineTime = $driverLocation->online_date;

    // Calculate the time difference in hours
    $currentDateTime = Carbon::now('Asia/Kolkata');
$hours = $currentDateTime->diffInHours($onlineTime); // Full hours
$minutes = $currentDateTime->diffInMinutes($onlineTime) % 60; // Remaining minutes

// Convert hours and minutes to a decimal format
$totalOfflineHours = $hours + ($minutes / 60); // e.g., 2.08 hours

// Retrieve the existing total hours from the database
$totalhours = !empty($driverLocation->total_hour) ? $driverLocation->total_hour : 0;
$totalhours += $totalOfflineHours; // Add the new offline time


// Update the total_hour field as a readable string (e.g., 2 hours and 12 minutes)
$driverLocation->total_hour = $totalhours;

// Save the updated total hours to the database
$driverLocation->save();
